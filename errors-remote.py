...
def find_error_for_rule(model, rule, packs, epc, bs, name):
    ...
    plt.savefig('diff-plot/rule-'+str(rule)+'-'+name+'.png', dpi=300)
    plt.clf()
            ...
            for batch_size in [64]:
                for epochs in [512]:
                    for afun in ['relu','selu','tanh']:
                        print(rule, packs, batch_size, epochs, afun, layers, find_error_for_rule(get_model(afun, layers), rule, packs, epochs, batch_size, afun+"_"+str(batch_size)+"_"+str(layers)))
                        sys.stdout.flush()
